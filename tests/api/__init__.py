...
from helium3 import (go_to, kill_browser, set_driver, start_chrome,
                     start_firefox)
...
class BrowserAT(TestCase):
    ...
    def read_result_from_browser(self, timeout_secs=3):
        ...
        while time() < start_time + timeout_secs:
            result = self.driver.find_element_by_id("result").get_attribute("innerHTML")
            if result:
                return result
            sleep(0.2)
        return ""
...
_TEST_BROWSERS = {"firefox": start_firefox, "chrome": start_chrome}
